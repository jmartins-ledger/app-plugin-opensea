            ...
            returns(bool)
    {
        ...
        assembly
        {
            let length : = mload(a)
                         ...
                         switch eq(length, mload(b)) case 1
            {
        ...
        if (source.length > 0)
        {
            assembly
            {
                ...
                eq(lt(arrIndex, end), 1){
                    ...
                    mstore(tempIndex, mload(arrIndex))} index : = add(index, length)
            }
        }
        return index;
    }
...
contract ExchangeCore is ReentrancyGuarded, Ownable
{
     ...
     */
    function staticCall(address target, bytes memory calldata, bytes memory extradata) public view
    returns(bool result)
    {
        ...
        {
        result:
            = staticcall(gas, target, add(combined, 0x20), mload(combined), mload(0x40), 0)
        }
        return result;
    }
...
contract Exchange is ExchangeCore
{
     ...
     */
    function guardedArrayReplace(bytes array, bytes desired, bytes mask) public pure
        returns(bytes)
    {
     ...
     * @return byte array
     */
    function testCopy(bytes arrToCopy) public pure
        returns(bytes)
    {
     ...
     * @return byte array
     */
    function testCopyAddress(address addr) public pure
        returns(bytes)
    {
     ...
     */
    function calculateFinalPrice(SaleKindInterface.Side side, SaleKindInterface.SaleKind saleKind, uint basePrice, uint extra, uint listingTime, uint expirationTime) public view
        returns(uint)
    {
    ...
    function hashOrder_(
        ...
        bytes calldata,
        bytes replacementPattern,
        bytes staticExtradata) public pure
        returns(bytes32)
    {
        return hashOrder(
    ...
    function calculateCurrentPrice_(
        ...
        bytes calldata,
        bytes replacementPattern,
        bytes staticExtradata) public view
        returns(uint)
    {
    ...
    function ordersCanMatch_(
        ...
        bytes staticExtradataSell) public view
    returns(bool)
    {
     ...
     */
    function orderCalldataCanMatch(bytes buyCalldata, bytes buyReplacementPattern, bytes sellCalldata, bytes sellReplacementPattern) public pure
    returns(bool)
    {
    ...
    function atomicMatch_(
        ...
        uint8[2] vs,
        bytes32[5] rssMetadata) public payable
    {
...
library SaleKindInterface
{
     ...
     */
    function
    validateParameters(SaleKind saleKind, uint expirationTime)
        pure
            internal
            returns(bool)
    {
