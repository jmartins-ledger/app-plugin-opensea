...
test("Unwrap nanos", async () => {
  ...
  try {
    ...
    await eth.setExternalPlugin("0x7f39c581f595b53c5cb19bd0b3f8da6c935e2ca0", "0xde0e9a3e"); // Send transaction
    ...
    (0, _jest.expect)(lido).toEqual(expected_lido); // Unwrap

    filename = "unwrap.png";
    await sim.clickRight(SNAPSHOT_PATH_NANOS + filename);

    const unwrap = _zemu.default.LoadPng2RGB(SNAPSHOT_PATH_NANOS + filename);

    const expected_unwrap = _zemu.default.LoadPng2RGB(ORIGINAL_SNAPSHOT_PATH_NANOS + filename);

    (0, _jest.expect)(unwrap).toEqual(expected_unwrap); // Max Fees
