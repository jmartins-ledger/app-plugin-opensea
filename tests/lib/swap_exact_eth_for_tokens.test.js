...
const abi = require(abi_path); // Reference transaction for this test:
// https://etherscan.io/tx/0x0160b3aec12fd08e6be0040616c7c38248efb4413168a3372fc4d2db0e5961bb
// Nanos S test
// EDIT THIS: build your own test


test('[Nano S] Swap Exact Eth For Tokens with beneficiary', (0, _test.zemu)("nanos", async (sim, eth) => {
  ...
  const beneficiary = "0x15557c8b7246C38EE71eA6dc69e4347F5DAc2104"; // EDIT THIS: adapt the signature to your method
  ...
  const serializedTx = _ethers.ethers.utils.serializeTransaction(unsignedTx).slice(2); // Rather than constructing the tx ourselves, one can can obtain it directly through etherscan.
  ...
  const tx = eth.signTransaction("44'/60'/0'/0", serializedTx); // Wait for the application to actually load and parse the transaction
...
test('[Nano X] Swap Exact Eth For Tokens with beneficiary', (0, _test.zemu)("nanox", async (sim, eth) => {
  ...
  await (0, _test.waitForAppScreen)(sim); // Navigate the display by pressing the right button 6 times, then pressing both buttons to accept the transaction.
